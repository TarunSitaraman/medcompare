...
import { getSaved, type SavedMedicine } from '@/lib/saved'
import { getThisMonthSavingsTotal } from '@/lib/savings'
...
export default function HomePage() {
  ...
  const [savedItems, setSavedItems] = useState<SavedMedicine[]>([])
  ...
  useEffect(() => {
    ...
    setMonthSavings(getThisMonthSavingsTotal())

    function onSavedUpdate() { setSavedItems(getSaved()) }
    function onSavingsUpdate() { setMonthSavings(getThisMonthSavingsTotal()) }
    window.addEventListener('medcompare:saved-updated', onSavedUpdate)
    window.addEventListener('medcompare:savings-updated', onSavingsUpdate)
    return () => {
      window.removeEventListener('medcompare:saved-updated', onSavedUpdate)
      window.removeEventListener('medcompare:savings-updated', onSavingsUpdate)
    }
  }, [])
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-surface)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1024, margin: '0 auto', padding: 'clamp(24px, 5vw, 56px) 20px clamp(28px, 5vw, 64px)', display: 'flex', alignItems: 'center', gap: 48, position: 'relative' }}>
          ...
          {/* My Medicines + Savings — below search chips */}
          {savedItems.length > 0 && (
            <div style={{ marginTop: 8 }}>
              {monthSavings > 0 && (
                <div style={{
                  background: 'linear-gradient(135deg, var(--gold-surface), oklch(97% 0.05 200))',
                  border: '1px solid var(--gold-border)', borderRadius: 18,
                  padding: '12px 16px', marginBottom: 10,
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12,
                }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--gold-text)' }}>
                      Saved ₹{monthSavings} this month
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>via generic alternatives</div>
                  </div>
                  <Link href="/cart" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--gold-surface)', color: 'var(--gold-text)', border: '1px solid var(--gold-border)', borderRadius: 12, padding: '7px 12px', fontSize: 12, fontWeight: 600, textDecoration: 'none', flexShrink: 0 }}>
                    View cart
                  </Link>
                </div>
              )}
              <SavedList
                ...
                onCountChange={count => { if (count === 0) setSavedItems([]) }}
              />
            </div>
          )}
